(function ($) {
  ...
  $(document).on("touchstart click", function (e) {
    ...
    if (!container.is(e.target)
      ...
      && !$(e.target).closest('.slicknav_menu').length) {
  ...
  $("[data-background]").each(function () {
    ...
    var imageUrlMobile = $(this).attr("data-background-mobile");
    var imageUrlTablet = $(this).attr("data-background-tablet"); // introduceți aici URL-ul imaginii redimensionate pentru mobil

    // Verificați dacă este disponibil URL-ul imaginii redimensionate pentru mobil
    if (imageUrlMobile !== "" || imageUrlTablet !== "") {
      // Folosiți media queries pentru a aplica imaginea redimensionată numai pe dispozitivele mobile
      var mq = window.matchMedia("(max-width: 767px)");
      var tq = window.matchMedia("(min-width:767px) and (max-width: 1023px)");  // schimbați aceasta valoare la dimensiunea dorită pentru dispozitivele mobile
      if (mq.matches) {
        $(this).css("background-image", "url(" + imageUrlMobile + ")");
      } else if (tq.matches) {
        $(this).css("background-image", "url(" + imageUrlTablet + ")");
      } else {
